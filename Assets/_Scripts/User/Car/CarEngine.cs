...
namespace _Project
{
    public class CarEngine : MonoBehaviour
    {
        ...
        [SerializeField] private float _driftMultiplier = 1.3f;
        [SerializeField] private float _driftMinSpeed = 12;

        [Header("Настройки ручника")]
        [SerializeField] private float _handbrakeDrag = 20f;
        [SerializeField] private float _handbrakeAngularDrag = 10f;
        [SerializeField] private float _handbrakeChangeSpeed = 5f;
        ...
        private void Movement()
        {
            ...
            if(_isHandbrake)
                _rigidbody.linearDamping = Mathf.Lerp(_rigidbody.linearDamping, _handbrakeDrag, _handbrakeChangeSpeed * Time.fixedDeltaTime);
            else
                _rigidbody.linearDamping = Mathf.Lerp(_rigidbody.linearDamping, _normalHandbrakeDrag, _handbrakeChangeSpeed * Time.fixedDeltaTime);
